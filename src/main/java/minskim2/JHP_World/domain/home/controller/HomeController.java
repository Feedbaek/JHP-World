...
public class HomeController {
    ...
    public String home(Model model) {
        ...
        List<AssignmentDto> problemSolvingList = assignmentService.findAssignmentByLectureId(problemSolving.getId(), 0, PREVIEW_SIZE);
        model.addAttribute("problemSolvingList", problemSolvingList);
        // 2. 자료구조 과제
        Lecture dataStructure = lectureService.findByName("자료구조");
        List<AssignmentDto> dataStructureList = assignmentService.findAssignmentByLectureId(dataStructure.getId(), 0, PREVIEW_SIZE);
        model.addAttribute("dataStructureList", dataStructureList);
        // 3. 알고리즘설계 과제
        Lecture algorithmDesign = lectureService.findByName("알고리즘설계");
        List<AssignmentDto> algorithmDesignList = assignmentService.findAssignmentByLectureId(algorithmDesign.getId(), 0, PREVIEW_SIZE);
        model.addAttribute("algorithmDesignList", algorithmDesignList);
        // 4. 오토마타 과제
        Lecture automata = lectureService.findByName("오토마타");
        List<AssignmentDto> automataList = assignmentService.findAssignmentByLectureId(automata.getId(), 0, PREVIEW_SIZE);
